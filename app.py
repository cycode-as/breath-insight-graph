...
SNORE_SCORE_THRESHOLD    = 0.10      # YAMNet score needed to call it Snoring
GASP_SCORE_THRESHOLD     = 0.15      # YAMNet score needed to call it Gasp
SPEECH_SCORE_THRESHOLD   = 0.35      # YAMNet score to call it Awake/Speech
...
class YAMNetModelManager:
        ...
        with self._lock:
            try:
                ...
                if gasp_score >= GASP_SCORE_THRESHOLD:
                    # Must not be dominated by speech (cough while awake ≠ apnea)
                    if speech_score < SPEECH_SCORE_THRESHOLD:
                        return 2  # Gasp/Apnea

                # Priority 2: Snoring
                if snore_score >= SNORE_SCORE_THRESHOLD:
                    # KEY FIX: if speech score is also high, person is AWAKE
                    # "hello hello" gives speech ~0.96, snore ~0.001 → correctly blocked
                    # Real snore gives snore ~0.6–0.9, speech ~0.01 → correctly passes
                    if speech_score >= SPEECH_SCORE_THRESHOLD:
                        print(f"[YAMNet] ⚠️  Snore suppressed — speech score too high "
                              f"({speech_score:.3f} ≥ {SPEECH_SCORE_THRESHOLD})")
                        return 3  # Awake/Speech (not snore)
    ...
    def _fallback_classify(self, audio_chunk: np.ndarray, energy: float) -> int:
        ...
        print(f"[FALLBACK] Snore energy: {snore_e:.1f}")
        if snore_e > 800:
            return 0
        return 1
...
def handle_snore_detected(dominant_hz: float):
    ...
    if count <= MAX_TILT_COUNT:
        ...
        serial_mgr.send("1")
        log_event("Step A", "Signal 1 sent to Arduino", f"Level {count}/{MAX_TILT_COUNT}")
        with state_lock:
        ...
        try:
            ...
            if vstage == 0:
                serial_mgr.send("1")
                log_event("Step C Result", "Snore persists → Signal 1 sent", "")
            else:
                serial_mgr.send("0")
                log_event("Step C Result",
                          f"Snore stopped → Signal 0 sent", "")
                with state_lock:
        ...
        serial_mgr.send("1")
        log_event("Escalation", "Signal 1 sent - Arduino handles haptic ", "")
        socketio.emit("status_update", build_status_payload())

# ─────────────────────────────────────────────────────────────────
# FSR READER THREAD
# ─────────────────────────────────────────────────────────────────
# NAYA — DIP switch Arduino pe hai, backend ko kuch nahi karna
def fsr_reader_thread():
    print("[FSR] DIP switch mode — position handled by Arduino internally")
    with state_lock:
        state["system_status"] = "SENSING"
        state["fsr_state"]     = "FSR_C"
    while True:
        time.sleep(10)
# ─────────────────────────────────────────────────────────────────
    ...
    while True:
        try:
            ...
            if stage == 0:   # Snoring
                ...
                socketio.emit("status_update", build_status_payload())
                serial_mgr.send("1")
                log_event("GASPING", "Apnea - Signal 1 sent", f"{dominant_hz:.0f}Hz")
                ...
                if state["system_status"] not in ("INFLATING", "SNORE_DETECTED"):
                    if state["inflation_level"] > 0 or state["snore_count"] > 0:
                        serial_mgr.send("0")
                        log_event("Silence", "Signal 0 sent - Arduino resetting", f"Was {state['inflation_level']}")
                        with state_lock:
                            state["snore_count"]     = 0
...
def download_history():
    serial_id = request.args.get("serial_id", state["current_serial_id"])
    ...
    try:
        if filter_date:
            rows = conn.execute(
                "SELECT timestamp, date, stage_num, label FROM history WHERE serial_id=? AND date=? ORDER BY timestamp DESC",
                (serial_id, filter_date)
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT timestamp, date, stage_num, label FROM history WHERE serial_id=? ORDER BY timestamp DESC",
                (serial_id,)
            ).fetchall()
    finally:
        conn.close()

    csv_data = "Date,Time,Stage,Detection\n"
    ...
    for row in rows:
        ...
        csv_data += f"{date_part},{time_part},{row['stage_num']},{row['label']}\n"
    ...
    return Response(
        ...
        headers={
            "Content-disposition":
            f"attachment; filename=sleep_history_{serial_id}.csv"
        }
    )
